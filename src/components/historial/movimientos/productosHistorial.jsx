...
const DATOS_MOCK_HISTORIAL = [
  {
    id: 1,
    producto_id: "prod_01",
    nombre_producto: "Shampoo Keratina Premium 1L",
    accion: "Editado",
    fecha: "2026-06-01T14:22:00",
    valores_anteriores: { precio: 85.00, stock: 15 },
    valores_actuales: { precio: 95.00, stock: 12 }
  },
  {
    id: 2,
    producto_id: "prod_02",
    nombre_producto: "Tinte Rubio Ceniza 8.1",
    accion: "Creado",
    fecha: "2026-05-31T09:15:00",
    valores_anteriores: null,
    valores_actuales: { precio: 32.50, stock: 40 }
  },
  {
    id: 3,
    producto_id: "prod_03",
    nombre_producto: "Cera Moldeadora Mate",
    accion: "Revertido",
    fecha: "2026-05-30T18:40:00",
    valores_anteriores: { precio: 45.00, stock: 8 },
    valores_actuales: { precio: 40.00, stock: 20 }
  },
  {
    id: 4,
    producto_id: "prod_04",
    nombre_producto: "Protector Térmico Spray",
    accion: "Eliminado",
    fecha: "2026-05-28T11:05:00",
    valores_anteriores: { precio: 62.00, stock: 5 },
  ...
  const handleRevertir = (log) => {
    try {
      if (log.accion === 'Editado' && log.valores_anteriores) {
        ...
        mostrarToast(`¡Cambios en "${log.nombre_producto}" revertidos con éxito (Simulado)!`, "success");
      }
    } catch (error) {
      console.error("Error al revertir:", error);
      mostrarToast(`No se pudo revertir el cambio de prueba`, "error");
    }
  };
                ...
                <tr key={m.id} className="bg-white dark:bg-[#121016] hover:bg-slate-50 dark:hover:bg-purple-950/20 transition-colors">
                  <td className="p-4">
                    <div className={`inline-flex items-center gap-2 px-3 py-1 rounded-full text-[10px] uppercase tracking-wider font-bold border ${
                      ...
                      <span>
                        {m.accion === 'Creado' ? '🟢' : m.accion === 'Editado' ? '✏️' : m.accion === 'Revertido' ? '↩️' : '🗑️'}
                      </span>
                      {m.accion}
                    </div> 
                  </td>
                  
                  <td className="p-4 font-semibold text-slate-900 dark:text-slate-100">{m.nombre_producto}</td>
                      ...
                      {(m.valores_anteriores?.stock !== undefined || m.valores_actuales?.stock !== undefined) && (
                        <div className="flex items-center gap-2 text-slate-600 dark:text-purple-300">
                          ...
                          {m.accion === 'Editado' ? (
                            <>
                              <span className="text-rose-500 dark:text-rose-400 line-through">
                                {m.valores_anteriores?.stock} un.
                              </span>
                              <span className="opacity-40">→</span>
                              <span className="text-emerald-600 dark:text-emerald-400 font-bold">
                                {m.valores_actuales?.stock} un.
                              </span>
                            </>
                          ) : (
                            <span className="text-emerald-600 dark:text-emerald-400 font-bold">
                              {m.valores_actuales?.stock || m.valores_anteriores?.stock} un.
                            </span>
                          )} 
                        </div> 
                      )} 
        ...
        <div className="fixed inset-0 z-50 bg-slate-900/40 dark:bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in duration-200">
          <div className="bg-white dark:bg-[#121016] border border-slate-200 dark:border-purple-950/50 p-6 rounded-2xl w-full max-w-md shadow-2xl animate-in zoom-in-95 duration-200">
            ...
            <div className="space-y-4 text-xs text-slate-600 dark:text-purple-200">
              <p>
                <span className="opacity-60 uppercase tracking-wider block text-[9px] text-slate-500 dark:text-purple-400">Producto:</span> 
                <strong className="text-slate-900 dark:text-white text-sm font-bold">{selectedLog.nombre_producto}</strong>
              </p>
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <div className="text-rose-500 dark:text-rose-400 text-base font-bold line-through">
                          {selectedLog.valores_anteriores?.precio ? `S/ ${Number(selectedLog.valores_anteriores.precio).toFixed(2)}` : '-'}
                        </div>  
                      </div>
                      ...
                      <div className="text-right">
                        ...
                        <div className="text-emerald-600 dark:text-emerald-400 text-base font-bold">
                          {selectedLog.valores_actuales?.precio ? `S/ ${Number(selectedLog.valores_actuales.precio).toFixed(2)}` : '-'}
                        </div> 
                      </div> 
                    </div> 
                  </div>
                  ...
                  <div className="bg-slate-50 dark:bg-[#121016]/60 border border-slate-200 dark:border-purple-950/40 rounded-xl p-4">
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        <div className="text-[9px] text-slate-400 dark:text-purple-500 uppercase tracking-wider">Antes</div>
                        <div className="text-rose-500 dark:text-rose-400 text-base font-bold line-through">
                          {selectedLog.valores_anteriores?.stock !== null ? `${selectedLog.valores_anteriores?.stock} un.` : '-'}
                        </div>  
                      </div>
                      <div className="text-slate-300 dark:text-purple-950 text-lg font-bold">→</div>
                      <div className="text-right">
                        <div className="text-[9px] text-slate-400 dark:text-purple-500 uppercase tracking-wider">Ahora</div>
                        <div className="text-emerald-600 dark:text-emerald-400 text-base font-bold">
                          {selectedLog.valores_actuales?.stock !== null ? `${selectedLog.valores_actuales?.stock} un.` : '-'}
                        </div>  
                      </div> 
                    </div> 
                  </div> 
